...
- **Headsign is uniformly terminal-station-name-based across every borough and line checked — never "Uptown & The Bronx"-style text, anywhere.** Pulled real headsigns for G (Brooklyn/Queens only, never touches Manhattan: `"Court Sq"`, `"Church Av"`), L (`"8 Av"`, `"Myrtle-Wyckoff Avs"`, `"Canarsie-Rockaway Pkwy"`), 6/5/4 (`"Pelham Bay Park"`, `"Brooklyn Bridge-City Hall"`, `"Eastchester-Dyre Av"`, `"Woodlawn"`, `"New Lots Av"`, etc.), M/N/Q (from the original walk-leg test above), and bus (`Q70-SBS` → `"LAGUARDIA LINK FARE FREE TERMINALS B-C"`). Cross-checked against real static bus GTFS `trips.txt` directly (not just OTP) — `trip_headsign` is well-populated there too (e.g. `M60+` → `"SELECT BUS LA GUARDIA AIRPORT"`, matching the bus's actual front display). Web research (see below) confirms *why* there's no borough split in this field: "Uptown & The Bronx" / "Downtown & Brooklyn" (and, outside Manhattan, "To Queens" / "To Brooklyn" / "To Manhattan") is real MTA **station-entrance** signage — which staircase to use before the turnstile — a categorically different sign from the **train headsign** (which train you're confirming you boarded), and only the latter exists in GTFS, uniformly, in every borough including Manhattan. This corrects an initial premise (that Manhattan might have richer directional data requiring special-case logic) — it doesn't, and neither does anywhere else, so no borough-conditional logic is needed or buildable.
...
| Typed street address | NYC Geoclient API, see below | New, this design — key obtained and live-verified, unblocked |
...
**Address geocoding (NYC Geoclient API) — key obtained and live-verified this session.** New `geocode_address(address: str) -> {lat, lon} | None` backend function and matching `find_address` tool (parallel to `find_stop`), calling NYC's Geoclient API. Scoped to NYC only, per your direction — this product's actual routing coverage (OTP's graph, GTFS feeds) is NYC-only anyway, so no plausibility/bounding-box check beyond what a NYC-scoped geocoder already guarantees.

Real, live-verified facts (not assumed from training data — tested directly against `api.nyc.gov` with the real key you provided, stored in `backend/.env` as `GEOCLIENT_API_KEY`, never committed):
- Endpoint: `GET https://api.nyc.gov/geoclient/v2/search.json?input={url-encoded free-text address}&subscription-key={key}` — auth is a plain query parameter, no special header needed. Confirmed working live.
- Success shape: top-level `status: "OK"`, `results: [{status: "EXACT_MATCH" | "POSSIBLE_MATCH" | ..., response: {latitude: <float>, longitude: <float>, firstBoroughName: <str>, ...~250 other GIS fields we don't need}}]`. `latitude`/`longitude` are real JSON floats (not the `xCoordinate`/`yCoordinate` NY State Plane fields also present in the response — those are a red herring for this use case, no `pyproj` transform needed, `latitude`/`longitude` are already WGS84 degrees, directly usable by `plan_route`). Verified against a real Cornell Tech address ("2 west loop road manhattan") → `EXACT_MATCH`, `(40.756552, -73.955881)`, `firstBoroughName: "MANHATTAN"` — genuinely correct.
- No-match shape: top-level `status: "REJECTED"`, `results: []` — a clean, unambiguous signal to build the honest-failure path against.
- A casual/colloquial input like "2 cornell tech place roosevelt island" was rejected; a properly boroughed input ("2 west loop road manhattan") matched. The `find_address` tool's description should tell the agent to include a real NYC borough name when it knows one, rather than a neighborhood nickname — improves match rate, still never fabricates a result when Geoclient itself returns `REJECTED`.
...
### 5. Mid-trip destination change — pure `SYSTEM_PROMPT` orchestration, zero new backend code

When the user changes their destination mid-conversation (e.g. "actually take me to X instead"), the flow is deliberately a live, user-initiated conversation, not a background re-plan — the agent can just ask, which the autonomous poll loop structurally can't. `SYSTEM_PROMPT` addition: on detecting a destination change, (1) call `cancel_monitored_trip` with no `trip_id` — it already auto-resolves the caller's own active trip via `list_active_trips` (built in Task 5 for the ambiguous-cancel case) or degrades gracefully if none exists; (2) if `user_location` isn't already available this session, ask the user for their current location (reusing the same geolocation mechanism from §2, not a new one); (3) call `plan_route` fresh from that current location to the newly-stated destination (resolved via `find_stop`/`find_address` same as any new trip request); (4) fall through to the existing proactive-monitoring-offer logic for the new trip. No new tool, no new Python — every primitive here already exists or is already designed above.

### 6. In-station "what to look for" signage — text ships now, graphical bullets blocked on assets

Every boarding/transfer narration states "board the {route} toward {headsign}" with `{headsign}` **bolded** in the chat response (markdown `**...**`, matching this codebase's existing bold-formatting convention already visible in live responses) — buildable and shipping in this pass, using the uniform real headsign data confirmed above.

**Graphical route bullets (colored circle/diamond icons matching real subway signage) are explicitly blocked** pending the icon/sign-service list you're providing. What's buildable now without the assets: a structured `route_short_name` + `headsign` pairing is already present in every leg the agent narrates (no new data plumbing needed) — once the icon assets exist, rendering them just means the frontend mapping `route_short_name` → icon asset for known values, falling back to plain bolded text for anything unmapped (never blocking on a missing icon, never fabricating one). This task's brief will build the bolded-text version now and leave a clearly-marked hook for the icon-rendering pass later, rather than half-building icon logic against assets that don't exist yet.
...
- `find_address`/Geoclient: unit tests mock the HTTP layer against the real confirmed response shapes above (`EXACT_MATCH`/`POSSIBLE_MATCH` success, `REJECTED` no-match) — matching this codebase's established convention (real data shape, mocked transport, e.g. `alerts.py`'s tests). No longer blocked — the key is live-verified and stored.
- Conversation-agent test confirming a destination-change turn calls `cancel_monitored_trip` with no `trip_id` before the new `plan_route` call, matching the existing dispatch-order test conventions from Phase 3 Task 5/8.

## Explicitly out of scope

- Origin-side walk in re-plan notifications (see above — a real architectural boundary, not an oversight).
- Platform-number/entrance-level guidance, in **any** borough including Manhattan (confirmed unavailable in MTA's static GTFS data via OTP — not a Manhattan-only gap, see the headsign research above; "Uptown & The Bronx"-style text is real station-entrance signage, categorically different from and never overlapping with the train-headsign data this design uses).
- Graphical route-bullet icons (matching real MTA signage colors/shapes) — blocked on the icon asset list; bolded text-only "what to look for" guidance ships in this pass, with the data contract already shaped to accept icons later without rework.
