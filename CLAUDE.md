...
5. **Local run on this laptop is an interim measure, not the intended home.** User will provide a Railway API key so the collector runs on Railway instead, decoupled from this machine's power/lid state (unresolved as of 2026-08-14: this machine's Windows power scheme has no detectable lid-close-action setting, likely vendor-software-managed — sleep-on-lid-close would pause collection, unverified either way). Once the Railway key arrives: deploy `backend/collectors/bus_collector.py` there as a long-running worker, keep the local copy only as a fallback, and this caveat becomes moot.
